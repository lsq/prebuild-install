...
const libc = process.env.LIBC || (os.type().startsWith('MINGW32_NT') ? 'gnu' : '') || (isAlpine(platform) ? 'musl' : '')
...
load.resolve = load.path = function (dir) {
  ...
  function resolve (dir) {
    ...
    const tuple = tuples.filter(matchTuple(platform, arch, libc)).sort(compareTuples)[0]
    if (!tuple) return
...
function parseTuple (name) {
  ...
  const [platform, archString, libc] = arr

  const architectures = archString.split('+')

  if (!platform) return
  if (!architectures.length) return
  if (!architectures.every(Boolean)) return

  return { name, platform, architectures, libc: libc || '' }
}

function matchTuple (platform, arch, libc) {
  return function (tuple) {
    if (tuple == null) return false
    if (tuple.platform !== platform) return false
    if (tuple.libc && tuple.libc !== libc) return false
